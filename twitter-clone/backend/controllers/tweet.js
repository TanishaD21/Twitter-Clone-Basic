const {tweets,users,follows,likes}=require("../drizzle/schema.js");
...
async function viewOurTweets(req, res) {
    ...
    const tweetIds = viewTweets.map((tweet) => tweet.id);
...
async function viewTweetsByUsername(req,res)
{
    try{
        const userId=req.user?.id;
        ...
        if(!username)
        {
            return res.status(404).json({message:"Provide a username"});
        }
        const tweetsByUsername=await db.select({id: tweets.id,
        content: tweets.content,
        createdAt: tweets.createdAt,
        updatedAt: tweets.updatedAt,
        userId: users.id,
        username: users.username,
        name: users.name}).from(tweets).innerJoin(users,eq(users.id,tweets.userId)).where(eq(users.username,username));
        if(tweetsByUsername.length==0)
        {
            return res.status(200).json({message:"No tweets found",tweets:tweetsByUsername});
        }
        return res.status(200).json({message:`View all the tweets by ${username}`,tweets:tweetsByUsername});
    }
    catch(error)
    {
        console.log("Error",error);
...
async function viewAllTweets(req,res)
{
    try{
        ...
        return res.status(200).json({message:"View All the tweets",AllTweets:viewAllTweets});
    }
    catch(error)
    {
        console.log("Error",error);
...
module.exports={
    createTweet,deleteTweet,updateTweet,viewOurTweets,viewTweetsByUsername,viewAllTweets
}